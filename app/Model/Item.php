	    ...
	    public function update_basket_totals($basket) {
	    	...
	    	foreach ($basket['items'] as $item_id => $value) {
    			...
    			if ($item_data['Item']['price'] == null) {
    				$basket['items'][$item_id]['sub_total'] = $value['quantity'] * $item_data['Item']['price_2'];
    			} else {
    				// считаем полные порции
    				$number_of_full_portions = floor($value['quantity'] / 8);
    				...
    				+ 	($value['quantity'] - ($number_of_full_portions * 8)) * $item_data['Item']['price']/4;
    			}
